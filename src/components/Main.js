    ...
    const signUp = async (email) => {
        try {
            const {data, error} = await supabase
                .from('users')
                .insert([
                    {first_name:"Bily", last_name:"Joe", email: email},
                ])
                .select()
        } catch (error) {
                ...
                <div className="max-w-7xl bg-black rounded-3xl mx-auto px-4 mb-44 sm:px-6 text-center">
                    <div className="py-24 md:py-24">
                        <div className="mx-auto max-w-screen-md sm:text-center">
                            <h2 className="mb-4 text-4xl tracking-tight font-extrabold text-white">Sign
                                up to get the app!</h2>
                            ...
                            <form action="#">
                                <div className="items-center mx-auto mb-3 space-y-4 max-w-screen-sm sm:flex sm:space-y-0">
                                    ...
                                    <div>
                                        <button 
                                            className="py-3 px-5 w-full text-sm font-medium text-center text-black rounded-lg border cursor-pointer bg-teal border-teal sm:rounded-none sm:rounded-r-lg hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
                                            onClick={handleClick}>
                                                Subscribe
                                        </button>
                                    </div>
